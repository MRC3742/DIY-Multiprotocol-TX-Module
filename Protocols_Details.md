...
- LT8910 PHY configuration from stock captures (`02b_1` TX + `52b` RX). Each SPI packet is 3 bytes: `REG`, `MSB`, `LSB` (CPHA=1 export). The table lists the first write to each register before the first payload marker (`0x32 00 00`).
...
- Bind flow chart (successful bind):
  - **TX (02b_1)**  
    1) Initialization writes (`0x00`–`0x2B`), then payload marker `0x32 00 00`.  
    2) Payload latch `0x34 80 80`, then payload bytes via `0x32` writes: `0A 00 11 22 33 06 AB FC AD 00`.  
    3) Hop loop: `0x07 01 <CH>` then `0x07 00 <CH>` for `00 28 0A 32 14 3C 1E 46` (repeats each bind burst).
  - **RX (52b)**  
    1) Initialization writes (table above) and payload marker `0x32 00 00`, followed by status reads (`0xA2`, `0xA8`, `0xC7`, `0xB0`, `0xF0`, `0x87`, etc.).  
    2) Channel scan writes on `0x03` and `0x07` include the hop set `00 28 0A 32 14 3C 1E 46`, extra scan values `06 08 16 30`, plus offset values `80 88 8A 94 96 9E A8 B0 B2 C6 CA D4 DE E8`.
